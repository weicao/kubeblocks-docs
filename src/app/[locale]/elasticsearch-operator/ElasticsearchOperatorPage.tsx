      ...
      <Container maxWidth="lg">
        ...
        >
          ...
          <Box>
            ...
            >
              KubeBlocks Elasticsearch Operator{' '}
            ...
            >
              Deploy production-grade Elasticsearch clusters in minutes.
              Multi-node HA, snapshot backup, horizontal scaling, and full Day-2 operations via a single operator.
            </Typography>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} mb={7}>
              ...
              <Button
                variant="outlined"
                href="/docs/preview/kubeblocks-for-elasticsearch/02-quickstart"
                size="large"
                sx={{ px: 4, py: 1.5 }}
              >
                Read the Docs
          ...
          >
            ...
            <Box sx={{ px: 3, py: 2.5, display: 'flex', flexDirection: 'column', gap: 0 }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mb: 1.25 }}>
                ...
                <Typography sx={{ fontSize: '13px', fontWeight: 600, color: '#8b949e' }}>Install Elasticsearch Addon</Typography>
              </Box>
              <Box component="pre" sx={{ m: 0, mb: 2.5, p: 2, borderRadius: '8px', bgcolor: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)', fontFamily: '"JetBrains Mono","Fira Code",monospace', fontSize: { xs: '0.68rem', md: '0.72rem' }, lineHeight: 1.7, color: '#c9d1d9', overflowX: 'auto' }}>
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25, mb: 1.25 }}>
                ...
                <Typography sx={{ fontSize: '13px', fontWeight: 600, color: '#8b949e' }}>Create an Elasticsearch Cluster</Typography>
              </Box>
              ...
              <Box component="pre" sx={{ m: 0, p: 2, borderRadius: '8px', bgcolor: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.06)', fontFamily: '"JetBrains Mono","Fira Code",monospace', fontSize: { xs: '0.68rem', md: '0.72rem' }, lineHeight: 1.7, color: '#c9d1d9', overflowX: 'auto' }}>
                ...
                {'es-cluster   elasticsearch     multi-node   '}
...
const topoData = [
  {
    name: 'Single Node',
    badgeColor: '#7C3AED',
    tags: ['Dev / Test', 'No HA', 'Fast Setup'],
    desc: 'A single Elasticsearch pod with all roles enabled (master, data, ingest, transform). Ideal for development, local testing, and small-scale workloads where high availability is not required.',
    features: [
      'All roles in one pod (master + data + ingest + transform)',
      'Minimal resource footprint — single CPU/memory allocation',
    ...
    diagram: <ElasticsearchSingleNodeDiagram />,
  },
  {
    name: 'MDIT (combined roles)',
    badgeColor: '#D97706',
    tags: ['No Dedicated Master', 'Scalable Replicas', 'Lower Config Complexity'],
    desc: 'Multiple Elasticsearch nodes each running all roles simultaneously (master, data, ingest, transform). Unlike single-node, you can scale out replicas for more throughput — but without dedicated master nodes, every node participates in master election.',
    features: [
      'All roles per node — no dedicated master nodes required',
      'Scale out replicas for higher throughput and capacity',
      'Master election across all nodes (any node can become master)',
      'Lower operational complexity than full role separation',
      'Suitable for medium workloads or teams new to Elasticsearch',
    ],
    diagram: <ElasticsearchMditDiagram />,
  },
  {
    name: 'Multi-Node (master + dit)',
    badgeColor: '#0165CB',
    tags: ['Production HA', 'Dedicated Masters', 'Shard Replication'],
    desc: 'Dedicated master nodes (3) for cluster state management, plus combined data+ingest+transform (dit) nodes for indexing and search. Master nodes use quorum-based cluster coordination — a quorum of 3 tolerates 1 master failure with zero data loss.',
    features: [
      'Dedicated master nodes for cluster state — no split-brain',
      'Data/ingest/transform nodes scale independently',
      'Shard replication across data nodes for fault tolerance',
      'Horizontal scaling of dit nodes for throughput',
      'Rolling version upgrades with quorum safety',
    ],
    diagram: <ElasticsearchArchitectureDiagram />,
  },
  {
    name: 'Full Separation (m-d-i-t)',
    badgeColor: '#059669',
    tags: ['Large Scale', 'Role Isolation', 'Optimal Perf'],
    desc: 'Fully separated node roles: master nodes (cluster state), data nodes (storage/search), ingest nodes (pipeline processing), and transform nodes (continuous transformations). Best for large-scale deployments where role-specific tuning is critical.',
    features: [
      'Master, data, ingest, and transform nodes independently sized',
      'Ingest nodes handle pipeline processing (no impact on data nodes)',
      'Transform nodes run continuous aggregations separately',
      'Each component scales horizontally via OpsRequest',
      'Optimal for large index counts and heavy ingest pipelines',
    ],
    diagram: <ElasticsearchFullSeparationDiagram />,
  },
];
      ...
      <Container maxWidth="lg">
        <Box textAlign="center" mb={6}>
          ...
          <Typography sx={{ color: 'text.secondary', mt: 1.5, maxWidth: 560, mx: 'auto', lineHeight: 1.75 }}>
            From a single-node dev instance to a fully role-separated production cluster —
...
const lifecycleCategories = [
  {
    ...
    color: '#0165CB',
    items: [
      { title: 'Horizontal Scaling', desc: 'Scale data, ingest, or transform node counts online without re-indexing. Cluster remains available throughout with minimal rebalancing overhead.' },
      { title: 'Vertical Scaling', desc: 'Resize CPU and memory on any role group with a rolling strategy that keeps the cluster available.' },
      { title: 'Volume Expansion', desc: 'Expand data node PVCs to increase index storage capacity without pod restarts.' },
      { title: 'Rolling Restart', desc: 'Controlled pod restarts with quorum-aware sequencing — master quorum is never broken.' },
      { title: 'Stop / Start', desc: 'Suspend clusters to eliminate compute cost; resume with full cluster state.' },
    ],
  },
  {
    ...
    color: '#059669',
    items: [
      { title: 'Dynamic Configuration', desc: 'Tune Elasticsearch parameters via OpsRequest — heap size, thread pool, circuit breakers.' },
      { title: 'Version Upgrade', desc: 'Rolling upgrades within the same major series with health checks. Cross-major upgrades supported when Elasticsearch compatibility prerequisites are met.' },
      { title: 'Prometheus Metrics', desc: 'Per-node metrics via elasticsearch-exporter on :9114 for Grafana dashboards.' },
      { title: 'Backup & Restore', desc: 'Snapshot-based physical backups to S3-compatible storage; restore from any snapshot.' },
      { title: 'Expose via LoadBalancer', desc: 'Enable external REST access for Kibana, Logstash, or application clients outside the cluster.' },
    ],
  },
];
  ...
  return (
    <Box component="section"
      ...
      <style>{`
        ...
        .cd-scale-pod.existing .cd-scale-pod-dot { background: #6CB6FF; }
      ...
      <Container maxWidth="lg">
        <Box sx={{ textAlign: 'center', mb: 6 }}>
          ...
          <Typography variant="h2"
            ...
            Built for{' '}
            <Box component="span" color="primary.main">Production Elasticsearch</Box>
          </Typography>
          <Typography sx={{ color: 'text.secondary', maxWidth: 560, mx: 'auto', lineHeight: 1.75 }}>
            KubeBlocks automates the hardest parts of running Elasticsearch on Kubernetes — so your team doesn&apos;t have to.
          </Typography>
        </Box>
          ...
          <div className="cd-card">
            <div className="cd-header">
              ...
              <div className="cd-title-group">
                ...
                <div className="cd-desc">Scheduled snapshot backups to S3-compatible object storage. Restore a full cluster to any stored snapshot in minutes.</div>
              </div>
              <div className="cd-badge cd-badge-orange">S3 Repository</div>
            </div>
            ...
            <div style={{ position: 'relative', marginBottom: '80px' }}>
              ...
              <div className="cd-tl-events">
                ...
                <div className="cd-tl-event" style={{ left: '88%' }}>
                  ...
                  <div className="cd-tl-time">20:35</div>
                </div>
              </div>
            </div>
            ...
            <div className="cd-restore">
              <div className="cd-restore-step">
                ...
                <div className="cd-restore-text">
                  ...
                  <span>Restore from nearest snapshot in S3 repository</span>
                </div>
              </div>
              <div className="cd-restore-arrow">→</div>
              <div className="cd-restore-step">
                <div className="cd-restore-icon blue">⚙️</div>
                <div className="cd-restore-text">
                  <strong>2. Restore Indices</strong>
                  <span>Recover index data and shard assignments</span>
                </div>
              </div>
              <div className="cd-restore-arrow">→</div>
              <div className="cd-restore-step">
                ...
                <div className="cd-restore-text">
                  ...
                  <span>New cluster started, master elected, shards green</span>
                </div>
              </div>
          ...
          <div className="cd-card">
            <div className="cd-header">
              ...
              <div className="cd-title-group">
                ...
                <div className="cd-desc">Data nodes are upgraded first while master nodes maintain the cluster state. Minimal index unavailability — search and ingest continue throughout.</div>
              </div>
            ...
            <div className="cd-upgrade-grid">
              <div className="cd-stage">
                ...
                <div className="cd-pod-grid">
                  <div className="cd-pod primary-old"><div className="cd-pod-dot"></div>master-0<div className="cd-pod-version cd-v-old">7.10.2</div></div>
                  <div className="cd-pod replica-old"><div className="cd-pod-dot"></div>dit-0<div className="cd-pod-version cd-v-old">7.10.2</div></div>
                  <div className="cd-pod replica-old"><div className="cd-pod-dot"></div>dit-1<div className="cd-pod-version cd-v-old">7.10.2</div></div>
                </div>
              </div>
              <div className="cd-stage-arrow">›</div>
              <div className="cd-stage highlight">
                <div className="cd-stage-label"><span>Step 2 — Data Nodes</span></div>
                <div className="cd-pod-grid">
                  <div className="cd-pod primary-old"><div className="cd-pod-dot"></div>master-0<div className="cd-pod-version cd-v-old">7.10.2</div></div>
                  <div className="cd-pod replica-new"><div className="cd-pod-dot"></div>dit-0<div className="cd-pod-version cd-v-new">7.17.0</div></div>
                  <div className="cd-pod upgrading"><div className="cd-pod-dot"></div>dit-1<div className="cd-pod-version cd-v-upg">upgrading…</div></div>
                </div>
              </div>
              <div className="cd-stage-arrow">›</div>
              <div className="cd-stage highlight">
                <div className="cd-stage-label"><span>Step 3 — Master</span></div>
                <div className="cd-pod-grid">
                  <div className="cd-pod upgrading"><div className="cd-pod-dot"></div>master-0<div className="cd-pod-version cd-v-upg">upgrading…</div></div>
                  <div className="cd-pod replica-new"><div className="cd-pod-dot"></div>dit-0<div className="cd-pod-version cd-v-new">7.17.0</div></div>
                  <div className="cd-pod replica-new"><div className="cd-pod-dot"></div>dit-1<div className="cd-pod-version cd-v-new">7.17.0</div></div>
                </div>
              </div>
              <div className="cd-stage-arrow">›</div>
              <div className="cd-stage">
                ...
                <div className="cd-pod-grid">
                  <div className="cd-pod primary-new"><div className="cd-pod-dot"></div>master-0<div className="cd-pod-version cd-v-new">7.17.0 ★</div></div>
                  <div className="cd-pod replica-new"><div className="cd-pod-dot"></div>dit-0<div className="cd-pod-version cd-v-new">7.17.0</div></div>
                  <div className="cd-pod replica-new"><div className="cd-pod-dot"></div>dit-1<div className="cd-pod-version cd-v-new">7.17.0</div></div>
                </div>
              </div>
            ...
            <div className="cd-traffic">
              <div className="cd-traffic-label">Search &amp; Ingest Traffic</div>
              <div className="cd-traffic-track"><div className="cd-traffic-fill"></div></div>
              <div className="cd-traffic-status">✓ Continuous During Upgrade</div>
            </div>
          </div>
        )}
          ...
          <div className="cd-card">
            <div className="cd-header">
              ...
              <div className="cd-title-group">
                ...
                <div className="cd-desc">Add or remove data nodes online. KubeBlocks triggers shard rebalancing automatically — new nodes absorb shards from existing nodes. Cluster remains available throughout the rebalancing phase.</div>
              </div>
              <div className="cd-badge cd-badge-green">No Re-indexing</div>
            </div>

            <div className="cd-scale-grid">
              <div className="cd-scale-box">
                <div className="cd-scale-title" style={{color:'#6CB6FF'}}>Scale-Out (1→3 data nodes)</div>
                <div className="cd-scale-desc">Add 2 more data nodes to increase search throughput and storage capacity</div>
                <div className="cd-scale-label">Before</div>
                <div className="cd-scale-before">
                  <div className="cd-scale-pod existing"><div className="cd-scale-pod-dot"></div>dit-0 (data)</div>
                </div>
                <div className="cd-scale-arrow">↓</div>
                <div className="cd-scale-label">After</div>
                <div className="cd-scale-after">
                  <div className="cd-scale-pod existing"><div className="cd-scale-pod-dot"></div>dit-0 (data)</div>
                  <div className="cd-scale-pod added"><div className="cd-scale-pod-dot"></div>dit-1 (data) ✦ new</div>
                  <div className="cd-scale-pod added"><div className="cd-scale-pod-dot"></div>dit-2 (data) ✦ new</div>
                </div>
                <span className="cd-scale-badge green">Search Capacity ↑ (ideal conditions)</span>
              </div>

              <div className="cd-scale-box">
                <div className="cd-scale-title" style={{color:'#4ADE80'}}>Auto Shard Rebalancing</div>
                <div className="cd-scale-desc">Elasticsearch redistributes shards to new nodes automatically — no manual intervention</div>
                <div className="cd-scale-label">Shard Distribution</div>
                <div className="cd-scale-after">
                  <div className="cd-scale-pod existing" style={{justifyContent:'space-between'}}><span><div className="cd-scale-pod-dot" style={{display:'inline-block',marginRight:'6px'}}></div>dit-0</span><span style={{fontSize:'10px',color:'#8B90A0'}}>P0, P1, P2 → P0</span></div>
                  <div className="cd-scale-pod added" style={{justifyContent:'space-between'}}><span><div className="cd-scale-pod-dot" style={{display:'inline-block',marginRight:'6px'}}></div>dit-1</span><span style={{fontSize:'10px',color:'#8B90A0'}}>— → P1</span></div>
                  <div className="cd-scale-pod added" style={{justifyContent:'space-between'}}><span><div className="cd-scale-pod-dot" style={{display:'inline-block',marginRight:'6px'}}></div>dit-2</span><span style={{fontSize:'10px',color:'#8B90A0'}}>— → P2</span></div>
                </div>
                <span className="cd-scale-badge green">✓ Shards Green After Rebalance</span>
              </div>
            </div>
            ...
            <div className="cd-traffic">
              ...
              <div className="cd-traffic-track"><div className="cd-traffic-fill"></div></div>
              <div className="cd-traffic-status">✓ Online — Rebalancing in Background</div>
            </div>
          </div>
        )}
...
function Comparison() {
  ...
  const rows = [
    ...
    { feature: 'Stop / start cluster',                    kb: YES,  eck: NO,   bitnami: NO   },
    {
      ...
      kb: YES,
      eck: PART,
      eckNote: 'Elastic License 2.0 — not OSI open source.',
      bitnami: YES,
    },
  ...
  const cols = [
    ...
    { label: 'ECK (Elastic)',       highlight: false },
      ...
      <Container maxWidth="lg">
        <Box textAlign="center" mb={6}>
          ...
          <Typography variant="h4" fontWeight={700} sx={{ letterSpacing: '-0.03em', lineHeight: 1.15 }}>
            ...
            <Box component="span" color="primary.main">Other Elasticsearch Options</Box>
          </Typography>
        </Box>
        ...
        <Box sx={{ overflowX: 'auto', borderRadius: '14px', border: `1px solid ${cardBorder}` }}>
          <Box component="table" sx={{ width: '100%', borderCollapse: 'collapse', bgcolor: cardBg }}>
            ...
            <Box component="tbody">
              {rows.map(({ feature, kb, eck, eckNote, bitnami }) => (
                <Box
                  key={feature}
                  component="tr"
                  sx={{
                ...
                >
                  ...
                  {[kb, eck, bitnami].map((val, j) => (
                    ...
                    >
                      {val}
                      {j === 1 && eckNote && (
                        <Box component="span" sx={{ display: 'block', fontSize: '0.68rem', color: 'text.disabled', mt: 0.5, lineHeight: 1.4 }}>
                          {eckNote}
                        </Box>
                      )}
                    </Box>
...
const esPosts = [
  ...
  {
    title: 'How Containerization Affects Database Performance: A Benchmark of runC, Kata, and gVisor',
    description: 'An in-depth benchmark of CPU, memory, and I/O for container runtimes, investigating common bottlenecks in a Kubernetes environment.',
    ...
    <Box sx={{ py: { xs: 6, md: 10 }, borderTop: '1px solid', borderColor: 'divider' }}>
      <Container maxWidth="lg">
        <Box textAlign="center" mb={6}>
          ...
          <Typography variant="h4" fontWeight={700} sx={{ letterSpacing: '-0.03em', lineHeight: 1.15 }}>
            Go Deeper on{' '}
            <Box component="span" color="primary.main">Elasticsearch on Kubernetes</Box>
          </Typography>
        </Box>
        ...
        <Box display="flex" flexWrap="wrap" gap={3}>
          <Box maxWidth={360}>
            ...
            <Typography variant="body2" color="text.secondary" lineHeight={1.6}>
              Kafka and Elasticsearch are often deployed together as the ELK-style
              log pipeline backbone. KubeBlocks manages both with a single operator
              and unified Day-2 operations.
            </Typography>
          </Box>
        </Box>
      </Container>
...
function CTA() {
  const checks = ['Open Source', 'Elasticsearch & 35+ other engines', 'Production-grade HA', 'No vendor lock-in'];
    ...
    <Box sx={{ borderTop: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
      <Container maxWidth="md">
        <Stack alignItems="center" textAlign="center" sx={{ py: { xs: 8, md: 12 } }} spacing={0}>
          ...
          >
            Get Started with KubeBlocks Elasticsearch Operator,{' '}
...
export default function ElasticsearchOperatorPage() {
  return (
    <>
      <Hero />
      <TrustedBy />
