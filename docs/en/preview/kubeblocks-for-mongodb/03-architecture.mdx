...
|-----------|------|---------|
| `mongodb` | 27017 | MongoDB database engine; participates in replica set replication and election; roleProbe runs `/tools/syncerctl getrole` inside this container |
...
**Config server pods** (3-node CSRS replica set):

| Container | Port | Purpose |
|-----------|------|---------|
| `mongodb` | 27017 | Config server mongod — stores chunk routing metadata; must use `w:majority` for all config writes; roleProbe runs `/tools/syncerctl getrole` |
| `mongodb-backup-agent` | — | Percona Backup for MongoDB (PBM) agent |
| `exporter` | 9216 | Prometheus metrics exporter |

**Shard pods** (each shard = independent replica set):

| Container | Port | Purpose |
|-----------|------|---------|
| `mongodb` | 27017 | Data shard mongod — stores documents assigned to this shard's chunk range; roleProbe runs `/tools/syncerctl getrole` |
| `mongodb-backup-agent` | — | Percona Backup for MongoDB (PBM) agent |
| `exporter` | 9216 | Prometheus metrics exporter |
