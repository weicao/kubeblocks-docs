...
|-----------|------|---------|
| `mysql` | 3306 | MySQL database engine; roleProbe runs `/tools/syncerctl getrole` inside this container |
...
Adding `proxysql` to the topology inserts a ProxySQL Component in front of MySQL. ProxySQL provides connection pooling, automatic read/write splitting, and query caching. Client traffic goes to `{cluster}-proxysql:6033` instead of directly to MySQL pods.
...
| Container | Port | Purpose |
|-----------|------|---------|
| `mysql` | 3306, 33061 | MySQL database engine; port 33061 is used for Group Replication communication (GCS — Group Communication System) |
...
Traffic routing is identical to Semisync. The `roleSelector: primary` mechanism on the ClusterIP service works the same way — the syncer roleProbe detects the current GR role and updates the pod label accordingly.

### MGR ± ProxySQL

The `mgr-proxysql` topology adds ProxySQL for connection pooling and read/write splitting, identical to the Semisync+ProxySQL variant.
...
| `{cluster}-mysql-headless` | Headless | 3306 | all MySQL pods |
| `{orc-cluster}-orchestrator` | ClusterIP | 80 | Orchestrator web UI + HTTP API (container listens on :3000, Service exposes :80 as `orc-http`) |
...
The `orc-proxysql` topology adds ProxySQL for connection pooling and read/write splitting, as in other variants.
...
|---------|------|---------|
| `root` | Superuser | Default administrative account |
| `kbadmin` | Superuser | KubeBlocks internal management |
| `kbdataprotection` | Admin | Backup and restore (xtrabackup, mysqldump) |
| `kbprobe` | Monitor (read-only) | Health check queries; role detection is handled by the lifecycle roleProbe (`syncerctl` / `orchestrator-client`), not by this account |
| `kbmonitoring` | Monitor | Prometheus metrics collection |
| `kbreplicator` | Replication | Binlog replication between primary and replicas |
| `proxysql` | Monitor | Used by ProxySQL for health checking and query routing (present in semisync/MGR topologies) |
