...
:::caution
**Known issue (addon v1.0.3 / KubeBlocks 1.0.2):** Reconfiguring may remain in `Running` state indefinitely. This is caused by a missing `ComponentParameter` CR for ClickHouse sharding components. Track progress at [kubeblocks-addons#xxx](https://github.com/apecloud/kubeblocks-addons). As a workaround, update parameters directly via `clickhouse-client` or the ClickHouse HTTP API inside the pod.
:::
