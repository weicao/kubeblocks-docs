...
Every RabbitMQ pod runs one container:

| Container | Purpose |
|-----------|---------|
| `rabbitmq` | RabbitMQ broker handling message routing, queues, exchanges, clustering, and metrics |

The `rabbitmq` container exposes the following ports:

| Port | Protocol | Purpose |
|------|----------|---------|
| 4369 | TCP | EPMD — Erlang Port Mapper Daemon, node discovery for Erlang clustering |
| 5672 | AMQP | Client message traffic |
| 15672 | HTTP | Web management console and REST API |
| 15674 | HTTP | WebSocket STOMP endpoint |
