...
2. Patroni's leader lease expires — **TTL depends on your Patroni / `bootstrap.dcs` settings** (often on the order of tens of seconds; consult your cluster config rather than assuming a fixed value).
3. A replica acquires leadership in DCS (with the Kubernetes DCS backend, this is coordinated through the Patroni `scope` objects in the API).
