...
:::note
MinIO requires a **minimum of 2 replicas**. For production, use at least 4 replicas distributed across multiple Kubernetes nodes. The replica count must always be a **multiple of 2**.
:::
...
- **Horizontal scale-out** — Add nodes (in multiples of 2)
...
- **Scale-in not supported** — Once nodes are added, they cannot be removed. MinIO's distributed erasure coding does not support pool removal.
- **Replica count must be a multiple of 2** — Erasure coding requires balanced stripe sizes
