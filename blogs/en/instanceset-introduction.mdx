...
title: 'InstanceSet: A Kubernetes Workload API for High-Availability Databases'
description: An introduction to InstanceSet, the general workload API in KubeBlocks designed for managing stateful applications. Learn about its key features, including role-based updates, in-place updates, and instance templates, which provide high availability for databases on Kubernetes.
date: 2024-08-28
authors:
  name: free6om
  url: https://github.com/free6om
  image_url: https://avatars.githubusercontent.com/u/28781141?v=4
tags: [Engineering, KubeBlocks, Internals]
Keywords: InstanceSet, KubeBlocks, Kubernetes, Workload API, StatefulSet, High Availability, In-Place Update, Database on Kubernetes, Role-Based Update, Instance Template, Pod Management, Horizontal Scaling, Stateful Workloads, Operator, Service Discovery
...
Most database systems support multi-instance deployments, where each instance assumes a different role, typically determined by their internal data replication relationships. For example, in [PostgreSQL](/docs/preview/kubeblocks-for-postgresql), there are Primary and Secondary roles, while in [etcd](/docs/preview/kubeblocks-for-etcd), there are roles like leader, follower, and learner.
...
- [KubeBlocks for etcd](/docs/preview/kubeblocks-for-etcd)