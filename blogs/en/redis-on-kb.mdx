---
slug: kubeblocks-x-redis-streaming-redis-cluster-and-solving-network-compatibility
title: 'KubeBlocks x Redis: Streamlining Redis Cluster and Solving Network Compatibility'
description: How we manage/operatorize Redis Cluster on Kubernetes with KubeBlocks and solve network compatibility issues (CNI, NodePort, announce-ip) when using Redis Cluster.
date: 2024-05-28
authors:
  name: dullboy
...
tags: [Addons, Redis]
keywords: redis, kubeblocks, operator, operatorization, redis cluster, kubernetes operator, cni, flannel, calico, nodeport, announce-ip, cluster-announce, network nat, high availability, statefulset
...
[Redis](/docs/preview/kubeblocks-for-redis) Cluster is the distributed solution for the Redis database, used to distribute data across multiple nodes to provide high availability and scalability. It allows storing a large amount of data sharded across multiple nodes and automatically handles data sharding and migration.
...
Many of Kubeblocks' clients have a strong demand for Redis Cluster, so we have adapted Redis Cluster based on Kubeblocks. During the adaptation process, we also discovered some network standard compatibility issues that Redis Cluster faces in the Kubernetes operatorized scenario.
